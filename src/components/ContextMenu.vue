...
const menuItems = {
  ...
  pinFolder: {
    ...
    action: () => {
      ...
      app.requester.pinFolder(selectedItems.value); // storage.setStore('pinned-folders', app.pinnedFolders);
    },
  },
  ...
  unpinFolder: {
    ...
    action: () => {
      app.pinnedFolders = app.pinnedFolders.filter((fav) => {
        const folder = selectedItems.value.find(
          (item) => item.path === fav.path
        );
        app.requester.unpinFolder(folder);

        return !selectedItems.value.find((item) => item.path === fav.path);
      });
