<template>
  <div class="vuefinder__breadcrumb__container">
    <span :title="t('Toggle Tree View')">
      <ListTreeSVG
        @click.native="toggleTreeView"
        class="vuefinder__breadcrumb__toggle-tree"
        :class="
          app.showTreeView ? 'vuefinder__breadcrumb__toggle-tree--active' : ''
        "
      />
    </span>
    ...
    <span :title="t('Go up a directory')">
      <GoUpSVG
        ...
        @dragleave.native="handleDragLeave($event)"
        @drop.native="handleDropZone($event)"
        @click.native="handleGoUp"
        :class="
    ...
    <span :title="t('Refresh')" v-if="!app.fs.loading">
      <RefreshSVG @click.native="handleRefresh" />
    </span>
    <span :title="t('Cancel')" v-else>
      <CloseSVG @click.native="app.emitter.emit('vf-fetch-abort')" />
    </span>
    ...
    >
      <div>
        <HomeSVG
          @dragover.native="handleDragOver($event)"
          @dragleave.native="handleDragLeave($event)"
          @drop.native="handleDropZone($event, -1)"
          @click.native="
            handleHomeClick;
    ...
    <div v-show="app.fs.searchMode" class="vuefinder__breadcrumb__search-mode">
      ...
      <input
        ...
        type="text"
      />
      <ExitSVG @click.native="exitSearchMode" />
    </div>
